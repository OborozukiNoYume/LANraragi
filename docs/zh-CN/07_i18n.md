...
|------|------|
| `footer.html.tt2` | 页脚 |
| `exception.production.html.ep` | 生产环境错误页面 |
...
| `config_shinobu.html.tt2` | Shinobu 文件监听设置 |
...
msgstr "标题已更改为: ${title}"
```
> 注意：`\` 仅在 msgid 中使用，msgstr 中不需要

### 特殊情况

`~` 是 Locale::Maketext 中的转义字符：
```
