...
Verified callers of `invalidate_cache()`: `set_tags()` and `set_isnew()` (`lib/LANraragi/Utils/Database.pm`), archive metadata edits (`update_metadata()` in `lib/LANraragi/Model/Archive.pm`), category membership changes (`lib/LANraragi/Model/Category.pm`), every tankoubon mutation *except* progress updates (`update_tank_progress()` rewrites the tank's `progress` metadata member without busting the cache, mirroring the archive behavior described below — `lib/LANraragi/Model/Tankoubon.pm`), Shinobu's file added/edited/deleted callbacks (`lib/Shinobu.pm`), backup restore (`lib/LANraragi/Model/Backup.pm`), uploads (`lib/LANraragi/Model/Upload.pm`), batch tagging (`lib/LANraragi/Controller/Batch.pm`), the `nHentaiSourceConverter` script plugin (`lib/LANraragi/Plugin/Scripts/nHentaiSourceConverter.pm`), and the search-cache endpoint (`clear_cache()` in `lib/LANraragi/Controller/Api/Search.pm`). The clear-new-flags endpoint (`clear_new_all()` in `lib/LANraragi/Controller/Api/Database.pm`) does *not* touch the search cache — it only resets `isnew` fields and drops `LRR_NEW`.
